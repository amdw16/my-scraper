/*************************************************************************
 * Scribely Alt‑Text Checker – v8.1 (srcset‑aware, endless‑scroll, adaptive)
 *************************************************************************/
...
/*──────────────────── Helpers ───────────────────────────*/
const chooseSrc = g => {
  /* take first URL token (handles "url 2x" syntax) */
  const raw =
    g('data-srcset') || g('srcset') ||              // ← srcset added
    g('data-src')    || g('data-lazy') ||
    g('data-original') || g('src') || '';
  return raw.split(',')[0].trim().split(' ')[0];    // first URL only
};

const normaliseSizeToken = s => s.replace(/\{width\}x\{height\}/gi, '600x');

/* … (buildReport, filterPlaceholders – unchanged) … */
...
function filterPlaceholders(list){
  ...
  return list.filter(it=>{
    ...
    const d=freq[it.src]||0, gif=/\.gif(?:$|\?)/i.test(it.src),
...
async function scrapeHtml(url){
  ...
  const raw=$('img').map((_,el)=>{
    const $el=$(el), w=+$el.attr('width')||0, h=+$el.attr('height')||0;
    return{
  ...
  async function runPass({jsOn,timeout,ua}){
    ...
    try{
      ...
      const raw=await page.$$eval('img',els=>els.map(el=>{
        ...
        const src=g('data-srcset')||g('srcset')||                // ← srcset added
                  g('data-src')||g('data-lazy')||
                  g('data-original')||g('src')||'';
        const w=el.width||0,h=el.height||0;
        return{src,alt:(g('alt')||'').trim(),tooSmall:w&&h&&(w*h<=9)};
      }));
      await browser.close();
