...
function wordsAround($, $el, limit) {
  ...
  return [...before.slice(-limit), ...after.slice(0, limit)]
         .join(' ')
         .toLowerCase();
}
        ...
        (els, lim) => {
          return els.map(el => {
            ...
            if (a) {
              ...
              const before = grab('previousSibling');
              const after  = grab('nextSibling');
              const around = [...before.slice(-lim), ...after.slice(0, lim)]
                              .join(' ').toLowerCase();
