/*************************************************************************
 * Scribely Alt‑Text Checker – v11
 *  • Rules: Missing‑Alt, File‑Name, Matching‑Nearby‑Content, Manual.
 *  • Duplicate‑text window: 50 words (deep=0) or 300 words (deep=1)
 *************************************************************************/
...
/******************************************************************
 * bucket() – builds response WITHOUT circular refs
 ******************************************************************/
function bucket(rawList, url){
  const groups = {
    'Missing Alt Text':        [],
    'File Name':               [],
  ...
  rawList.forEach(img=>{
    ...
    // keep only serialisable props
    const clean = { src: img.src, alt: img.alt };
    if(img.matchingSnippet) clean.matchingSnippet = img.matchingSnippet;

    if(!img.alt)
      groups['Missing Alt Text'].push(clean);
    else if(img.dup)
      groups['Matching Nearby Content'].push(clean);
    else if(altLower===baseName.toLowerCase() || extRE.test(altLower))
      groups['File Name'].push(clean);
    else
      groups['Manual Check'].push(clean);
  });

  return { totalImages: rawList.length, errorGroups: groups };
}
...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    if(domReport.totalImages<20){
      return res.status(200).json({...first,engine:'html',blocked:true,
