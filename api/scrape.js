/*************************************************************************
 * Scribely Alt-Text Checker – v18  (2025-04-29)
 * ----------------------------------------------------------------------
 * • JS/HTML time-outs increased (HTML 12 s, JS 12 s → fallback 20 s)
 * • Navigation / fetch time-outs are re-labelled as "blocked"
 * • Matching-text normalisation retained from v17
 *************************************************************************/
...
async function scrapeHTML(url,N){
  ...
  }catch{ throw new Error('blocked'); }            // network/DNS/timeout → blocked
...
/*──────── JS-DOM pass (12 s → fallback 20 s) ────────*/
async function scrapeDOM(url,N){
  ...
  try{
    return await run({jsOn:true, timeout:12000, ua:UA_PC});
  }catch(e){
...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    const needDom = placeholder>=0.8 || report.totalImages<MIN_IMG;
    ...
    /* turn any nav / fetch timeouts into "blocked" */
